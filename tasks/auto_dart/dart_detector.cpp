...
bool DartDetector::isCircle(const std::vector<cv::Point> & contour)  // 判断这个轮廓是不是圆形
{
  ...
  // 仅使用下限过滤，避免将较大目标误判为噪声（旧配置中的 max_area_ 偏小）
  if (area < min_area_) return false;               // 排除过小的干扰点
...
std::vector<LightSpot> DartDetector::detect(const cv::Mat & img)
{
  ...
  if (debug_) {
    ...
    tools::logger()->debug("Found {} raw contours", contours.size());
  }
  ...
  for (const auto & contour : contours) {  // 找到圆形的轮廓保存到circles当中
    ...
    double circularity = (4 * PI * area) / (perimeter * perimeter + 1e-12);
    bool pass = isCircle(contour);
...
io::DartCommand DartDetector::aim(const LightSpot lightspot, double offset)
{
  ...
  double yaw = atan2(tvec.at<double>(0), tvec.at<double>(2));  //actan(x/z)
  tools::logger()->info("send yaw:{}", -yaw - yaw_offset_ / 57.3 - offset);
  return {
    -yaw - yaw_offset_ / 57.3 - offset};  //-yaw - 安装误差(相机没有正对云台中心（装歪了）(相机偏右5度)) - 当前偏移（电控传的）
    //yaw左负右正（相机）
}

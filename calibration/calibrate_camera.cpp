  ...
  std::vector<std::vector<cv::Point2f>> & img_points)
{
  ...
  // 扫描目录中所有图片文件，按文件名数字排序（例如 1.jpg, 2.jpg ...），避免因缺号而停止
  namespace fs = std::filesystem;
  std::map<int, std::string> indexed_files;
  try {
    for (auto & entry : fs::directory_iterator(input_folder)) {
      ...
      if (lext != ".jpg" && lext != ".jpeg" && lext != ".png" && lext != ".bmp") continue;
      auto stem = p.stem().string();
      try {
        int idx = std::stoi(stem);
        indexed_files[idx] = p.string();
      } catch (...) {
        // 非数字命名的文件忽略
        continue;
      }
    ...
    fmt::print("Failed to scan folder {}: {}\n", input_folder, e.what());
    return;
  }

  if (indexed_files.empty()) {
    fmt::print("No images found in folder: {}\n", input_folder);
    return;
  }

  for (const auto & kv : indexed_files) {
    const auto & img_path = kv.second;
    auto img = cv::imread(img_path);
    if (img.empty()) {
      fmt::print("Warning: could not open image {} - skipping\n", img_path);
      continue;
    }
...
int main(int argc, char * argv[])
{
  ...
  if (obj_points.empty()) {
    fmt::print("Error: no valid calibration images found in '{}' - aborting\n", input_folder);
    return -1;
  }
